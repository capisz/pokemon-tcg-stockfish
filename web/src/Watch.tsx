...
import type { Analysis, Card, Deck, Job, ModelOption, PositionSummary, Replay, ReplaySummary, SavedPosition, ViewFrame } from './types';

type Source = {kind:'job'|'replay'|'position';id:string};
function rememberedSource():Source|null{try{const value=JSON.parse(sessionStorage.getItem('ptcg-watch-source')??'null');return value&&['job','replay','position'].includes(value.kind)&&typeof value.id==='string'&&/^[a-zA-Z0-9_-]+$/.test(value.id)?value:null;}catch{return null;}}
...
export function Watch(){
 ...
 const feed=useFrameFeed(source?.kind==='job'?`/jobs/${source.id}/frames?playerId=${perspective}`:null);
 const frames=source?.kind==='job'?feed.frames:savedFrames;
 const live=source?.kind==='job'&&!['finished','truncated','completed','error','failed','interrupted'].includes(feed.status);
 const playback=usePlayback(frames,`${source?.kind}:${source?.id}:${perspective}`,live);
 usePlaybackKeys(playback);
 const observation=playback.frame?.observation??null;
 const replayId=source?.kind==='replay'?source.id:source?.kind==='job'?feed.replayId:undefined;
 const selectedModel=models.find(m=>m.id===policy);
 const policyContext=source?.kind==='job'?feed.policyContext:replay?.policyContext;
 ...
 useEffect(()=>{if(!feed.replayId)return;const controller=new AbortController();void loadLibraries().catch(e=>setError(errorMessage(e)));api<Replay>(`/replays/${feed.replayId}?playerId=${perspective}`,{signal:controller.signal}).then(setReplay).catch(e=>{if(!controller.signal.aborted)setError(errorMessage(e));});return()=>controller.abort();},[feed.replayId,perspective,loadLibraries]);
 useEffect(()=>{
  setInspect(null);setAnalysis(null);setAnalysisError('');setPosition(null);setReplay(null);setSavedFrames([]);
  if(!source||source.kind==='job')return;
 ...
 const result=outcome?(outcome.winner===null?`Rules draw · ${outcome.reason}`:`Player ${outcome.winner+1} won · ${outcome.reason}`):replay?.status==='truncated'||feed.status==='truncated'?'Computation limit reached · outcome unknown':feed.status==='interrupted'?'Run interrupted · outcome unknown':live?'Simulation running':source?.kind==='job'?`${feed.status} · replay saved when available`:replay?.status;
 return <><header className="app-header"><h1>TCG Engine Lab</h1><p>Watch games and study decisions</p></header><main className="watch-main">
  ...
  </form><details className="support-notes"><summary>Selected deck support</summary>{deckIds.map((id,i)=>{const d=decks.find(d=>d.id===id);return d&&<div key={i}><strong>Player {i+1}: {d.validation.status}</strong>{[...d.validation.notes,...(d.support?.warnings??[])].map((n,j)=><p key={j}>{n}</p>)}</div>;})}</details></details>
  <div className="watch-library"><label>Saved replay<select value={replayId??''} onChange={e=>{if(e.target.value){setSource({kind:'replay',id:e.target.value});setNotice('');}}}><option value="">Choose a replay</option>{replays.map(r=><option key={r.id} value={r.id}>{r.id} · {r.status} · {r.frames} positions</option>)}</select></label><label>Saved position<select value={position?.id??''} onChange={e=>{if(e.target.value)setSource({kind:'position',id:e.target.value});}}><option value="">Choose a position</option>{positions.map(p=><option key={p.id} value={p.id}>{p.title}</option>)}</select></label><label>Perspective<select value={perspective} disabled={source?.kind==='position'} onChange={e=>setPerspective(Number(e.target.value))}><option value={0}>Player 1</option><option value={1}>Player 2</option></select></label></div>
  ...
  <div className="arena-layout"><div className="arena-stage">
   ...
   {observation?<CardTable observation={observation} previous={playback.previous} inspect={setInspect}/>:<div className="empty-playmat" role="status"><h2>{loading?'Opening position…':source?'Waiting for the first position…':'Watch a game unfold'}</h2><p>{source?'The board updates from the simulator’s recorded decisions.':'Choose two decks above, or open a saved replay. Only the selected player’s hand is visible.'}</p></div>}
   {source?.kind!=='position'&&<PlaybackControls playback={playback} live={source?.kind==='job'} status={feed.status}/>}
   {position?<div className="position-actions"><button onClick={()=>{setPerspective(position.playerId);setSource({kind:'replay',id:position.sourceReplayId});}}>Open source replay</button><span>Saved Player {position.playerId+1} information only</span></div>:<div className="position-actions"><button disabled={!replayId||!observation} onClick={()=>void save()}>Save position</button><span>{notice||(!replayId&&source?'Saving positions becomes available when the run is persisted.':'Left / Right to step. Playback pause leaves the simulation running.')}</span></div>}
  </div><aside className="arena-aside" aria-label="Study panel">{policyContext&&<section className="policy-context" aria-label="Simulation policy"><h3>{policyContext.policy==='model'?'Frozen local checkpoint':policyContext.policy==='random'?'Random legal baseline':'Heuristic baseline'}</h3><p>{policyContext.trainingStatus?.replaceAll('-',' ')}</p><details><summary>Policy and compute</summary><p>{policyContext.modelVersion}</p><p>{policyContext.opponentPopulation}</p><p>{policyContext.computeBudget}</p></details><p>Simulation does not train a model.</p></section>}{inspect&&<CardInspector card={inspect} close={()=>setInspect(null)}/>}<EvaluationPanel analysis={analysis} loading={analyzing} error={analysisError} retry={()=>setAnalysisRetry(v=>v+1)} observation={observation} playerId={perspective}/>{observation&&!analysis&&!analyzing&&!analysisError&&<p className="small-copy aside-note">{playback.playing?'Pause playback to analyze this position.':!replayId&&!position?'Analysis becomes available after this run is saved.':'Select a decision to study its alternatives.'}</p>}{replay&&<details className="aside-note"><summary>Replay provenance</summary><p>{replay.engineVersion}</p><p>{replay.policyContext?.policy??replay.policy??'Policy recorded in experiment metadata'}{replay.policyContext?.modelVersion?` · ${replay.policyContext.modelVersion}`:''}</p><p>Simulation does not train a model.</p>{replay.warnings.map((warning,i)=><p key={i}>{warning}</p>)}</details>}</aside></div>
