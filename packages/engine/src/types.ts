export interface CardView {
  id: string; name: string; kind: 'pokemon' | 'trainer' | 'energy'; types?: string[];
  hp?: number; stage?: string; prizeValue?: number;
  attacks?: {name: string; damage: number; cost: string[]}[];
}
...
export interface LegalAction { id: string; label: string; type: string; cardId?: string; target?: string }
export interface Observation {
  ...
  legalActions: LegalAction[]; history: string[]; prompt?: {type: string; message: string; cards?: CardView[]; hands?: CardView[][]}; warnings: string[];
}
...
export interface Replay {
  schemaVersion: 1; id: string; seed: number; decks: string[]; engineVersion: string;
