...
Profiles: Mac 2 workers, 8GiB aggregate memory target,25GiB data; Windows starts2 workers,40GiB aggregate memory target,200GiB data. Preserve20GiB free per destination. Desktop tuning limit=max(1,min(8,logicalCPUs-2)); increase only with measured benefit. CPU is the Windows baseline; Mac benchmarks CPU/MPS. Jobs are manually launched and resumable, with no scheduler. Quota/drive/process failures pause, not invent results.
