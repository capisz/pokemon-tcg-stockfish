...
`continuous --batches 0` runs successive bounded batches until interrupted. Each batch freezes an opponent population containing random/heuristic baselines and compatible champion/historical checkpoints. It gathers complete games with optional search-policy targets, trains for one epoch, and periodically records a paired evaluation. Evaluation is experimental and does not automatically install a champion. Each evaluation first creates unique, checksummed checkpoint snapshots under managed storage. Every game and champion publication uses those frozen bytes; changing the original training checkpoint does not change an in-progress evaluation. Snapshot hashes and original source paths remain in the protocol/report, and snapshots travel with experiment bundles. Explicit promotion requires coverage of every registered competitive list and all five archetypes in main, training-variant, and held-out roles; validating only a subset cannot satisfy the gate.
